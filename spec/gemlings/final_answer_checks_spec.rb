...
RSpec.describe "final_answer_checks" do
  ...
  it "passes when all checks return truthy" do
    ...
    agent = Gemlings::CodeAgent.new(model: model, final_answer_checks: [check])
  ...
  it "rejects answer and retries when a check fails" do
    ...
    check = ->(answer, _mem) { answer.include?("4") }
    agent = Gemlings::CodeAgent.new(model: model, final_answer_checks: [check], max_steps: 5)
    expect(agent.run("What is 2+2?")).to eq("4")
  ...
  it "evaluates multiple checks in order" do
    ...
    agent = Gemlings::CodeAgent.new(model: model, final_answer_checks: [check1, check2])
  ...
  it "provides memory to checks" do
    ...
    agent = Gemlings::CodeAgent.new(model: model, final_answer_checks: [check])
    agent.run("Test")
    expect(received_memory).to be_a(Gemlings::Memory)
  end
  ...
  it "includes check number in rejection message" do
    ...
    check1 = ->(_a, _m) { true }
    check2 = ->(a, _m) { a == "good" }
    agent = Gemlings::CodeAgent.new(model: model, final_answer_checks: [check1, check2], max_steps: 5)
    agent.run("Test")
