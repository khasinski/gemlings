...
RSpec.describe "Agent#step" do
  ...
  it "raises MaxStepsError when exceeded" do
    ...
    agent = Gemlings::ToolCallingAgent.new(model: model, max_steps: 2)
    agent.step("Test")
    agent.step
    expect { agent.step }.to raise_error(Gemlings::MaxStepsError)
  end
  ...
  it "accepts user messages on subsequent calls" do
    ...
    user_messages = agent.memory.steps.select { |s| s.is_a?(Gemlings::UserMessage) }
  ...
  it "validates final answer with checks" do
    ...
    agent = Gemlings::ToolCallingAgent.new(model: model, final_answer_checks: [check], max_steps: 5)
    agent.step("Test")
    expect(agent.done?).to be false
