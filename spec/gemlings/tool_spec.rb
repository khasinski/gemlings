...
RSpec.describe Gemlings::Tool do
  describe "DSL" do
    let(:tool_class) do
  ...
  describe "Gemlings.tool block shorthand" do
    let(:tool) do
      Gemlings.tool(:greet, "Greets a person", name: "The person's name") do |name:|
        "Hello, #{name}!"
      end
    end
  ...
  describe Gemlings::ManagedAgentTool do
    describe ".for" do
      it "creates an anonymous subclass per agent with isolated class state" do
        agent1 = instance_double(Gemlings::Agent, name: "agent_one", description: "First agent")
        agent2 = instance_double(Gemlings::Agent, name: "agent_two", description: "Second agent")
  ...
  describe Gemlings::FinalAnswerTool do
