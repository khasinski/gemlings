...
RSpec.describe Gemlings::Callback do
  ...
  describe "Agent callback integration" do
    let(:model) do
      instance_double(Gemlings::Model).tap do |m|
        allow(m).to receive(:generate).and_return(
          Gemlings::ChatMessage.new(
            role: "assistant",
            content: "Thought: done\nCode:\n```ruby\nfinal_answer(\"42\")\n```",
            token_usage: Gemlings::TokenUsage.new(input_tokens: 10, output_tokens: 5)
          )
        )
      end
    end
    ...
    it "fires on_run_start and on_run_end during a run" do
      ...
      agent = Gemlings::CodeAgent.new(model: model, callbacks: [cb])
      agent.run("test task")
    ...
    it "fires on_tool_call in ToolCallingAgent" do
      ...
      tc_model = instance_double(Gemlings::Model)
      ...
      agent = Gemlings::ToolCallingAgent.new(model: tc_model, callbacks: [cb])
      agent.run("test")
