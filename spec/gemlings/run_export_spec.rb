...
RSpec.describe "Run export (to_h / to_json)" do
  let(:token_usage) { Gemlings::TokenUsage.new(input_tokens: 100, output_tokens: 50) }

  describe Gemlings::TokenUsage do
  ...
  describe Gemlings::ToolCallFunction do
    it "serializes to hash" do
      f = Gemlings::ToolCallFunction.new(name: "search", arguments: { q: "ruby" })
      expect(f.to_h).to eq({ name: "search", arguments: { q: "ruby" } })
    end
  end

  describe Gemlings::ToolCall do
    it "serializes to hash including function" do
      tc = Gemlings::ToolCall.new(
        id: "call_1",
        function: Gemlings::ToolCallFunction.new(name: "echo", arguments: { msg: "hi" })
      )
      h = tc.to_h
  ...
  describe Gemlings::ActionStep do
    it "serializes with type discriminator" do
      step = Gemlings::ActionStep.new(
    ...
    it "omits nil optional fields" do
      step = Gemlings::ActionStep.new(step_number: 1, thought: "thinking")
      h = step.to_h
    ...
    it "serializes tool_calls" do
      tc = Gemlings::ToolCall.new(
        id: "c1",
        function: Gemlings::ToolCallFunction.new(name: "web", arguments: {})
      )
      step = Gemlings::ActionStep.new(step_number: 1, thought: "t", tool_calls: [tc])
      expect(step.to_h[:tool_calls]).to eq([{ id: "c1", function: { name: "web", arguments: {} } }])
    end
  end

  describe Gemlings::PlanningStep do
    it "serializes with type discriminator" do
      step = Gemlings::PlanningStep.new(plan: "Step 1\nStep 2", duration: 2.0, token_usage: token_usage)
      h = step.to_h
  ...
  describe Gemlings::UserMessage do
    it "serializes with type discriminator" do
      msg = Gemlings::UserMessage.new(content: "hello")
      expect(msg.to_h).to eq({ type: "user_message", content: "hello" })
    end
  end

  describe Gemlings::Memory do
    it "serializes to hash and JSON" do
      mem = Gemlings::Memory.new(system_prompt: "You are helpful", task: "Count to 3")
  ...
  describe Gemlings::RunResult do
    it "serializes to hash and JSON" do
      step = Gemlings::ActionStep.new(step_number: 1, thought: "done", observation: "42")
      result = Gemlings::RunResult.new(
