...
RSpec.describe "Integration with Ollama", :ollama do
  ...
  before do
    ...
    allow(Gemlings::UI).to receive(:step_header).and_call_original
  ...
  describe "CodeAgent" do
    it "solves a simple math problem" do
      agent = Gemlings::CodeAgent.new(model: "ollama/qwen2.5:3b", max_steps: 5)
    ...
    it "returns RunResult with timing and token info" do
      agent = Gemlings::CodeAgent.new(model: "ollama/qwen2.5:3b", max_steps: 3)
  ...
  describe "ToolCallingAgent" do
    it "solves a simple task with tool calling" do
      agent = Gemlings::ToolCallingAgent.new(model: "ollama/qwen2.5:3b", max_steps: 5)
