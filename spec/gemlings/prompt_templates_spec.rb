...
RSpec.describe "prompt customization" do
  ...
  describe Gemlings::PromptTemplates do
    it "defaults all fields to nil" do
      pt = Gemlings::PromptTemplates.new
    ...
    it "accepts keyword arguments" do
      pt = Gemlings::PromptTemplates.new(system_prompt: "custom")
  ...
  describe "instructions" do
    it "appends instructions to system prompt" do
      ...
      agent = Gemlings::CodeAgent.new(model: model, instructions: "Always answer in French.")
      agent.run("Hello")
    ...
    it "appends instructions for ToolCallingAgent" do
      response = Gemlings::ChatMessage.new(
        ...
        token_usage: Gemlings::TokenUsage.new(input_tokens: 10, output_tokens: 20),
        tool_calls: [
          Gemlings::ToolCall.new(
            id: "call_1",
            function: Gemlings::ToolCallFunction.new(name: "final_answer", arguments: { "answer" => "ok" })
          )
        ]
      )
      allow(model).to receive(:generate).and_return(response)

      agent = Gemlings::ToolCallingAgent.new(model: model, instructions: "Be concise.")
      agent.run("Test")
  ...
  describe "custom system prompt" do
    it "substitutes {{tool_descriptions}} in custom prompt" do
      ...
      templates = Gemlings::PromptTemplates.new(system_prompt: "Custom agent. Tools: {{tool_descriptions}}")
      agent = Gemlings::CodeAgent.new(model: model, prompt_templates: templates)
      agent.run("Test")
  ...
  describe "custom planning templates" do
    it "uses custom planning_initial prompt" do
      plan_response = Gemlings::ChatMessage.new(
        role: "assistant",
      ...
      templates = Gemlings::PromptTemplates.new(planning_initial: "Custom planner")
      agent = Gemlings::CodeAgent.new(model: model, prompt_templates: templates, planning_interval: 3)
      agent.run("Test")
    end
    ...
    it "uses custom planning_update prompt" do
      ...
      work_response = Gemlings::ChatMessage.new(
        role: "assistant",
      ...
      templates = Gemlings::PromptTemplates.new(planning_update: "Updated planner")
      agent = Gemlings::CodeAgent.new(model: model, prompt_templates: templates, planning_interval: 2, max_steps: 5)
      agent.run("Test")
    end
  end
end
