...
RSpec.describe Rubyagent::Sandbox do
  let(:final_answer_tool) { Rubyagent::FinalAnswerTool.new }
  ...
  describe "tool injection" do
    it "makes tools available as methods" do
      test_tool = Rubyagent.tool(:test_echo, "Echoes input", text: "text") { |text:| "echo: #{text}" }
