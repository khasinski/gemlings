...
RSpec.describe Rubyagent::MCP do
  describe Rubyagent::MCP::StdioTransport do
    it "performs handshake and accepts requests" do
      transport = Rubyagent::MCP::StdioTransport.new(command: ["ruby", MOCK_SERVER])
  ...
  describe Rubyagent::MCP::MCPToolWrapper do
    it "maps MCP tool schema to rubyagent input DSL" do
      mcp_tool = {
        "name" => "greet",
      ...
      client = instance_double(Rubyagent::MCP::StdioTransport)
      wrapper = Rubyagent::MCP::MCPToolWrapper.for(mcp_tool, client)
  ...
  describe "Rubyagent.tools_from_mcp" do
    it "loads tools from mock MCP server end-to-end" do
      tools = Rubyagent.tools_from_mcp(command: ["ruby", MOCK_SERVER])
  ...
  describe "Rubyagent.tool_from_mcp" do
    it "loads a single tool by name" do
      tool = Rubyagent.tool_from_mcp(command: ["ruby", MOCK_SERVER], tool_name: "echo")
    ...
    it "raises ArgumentError for unknown tool name" do
      expect {
        Rubyagent.tool_from_mcp(command: ["ruby", MOCK_SERVER], tool_name: "nonexistent")
