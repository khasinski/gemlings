...
RSpec.describe Rubyagent::Tool do
  describe "DSL" do
    let(:tool_class) do
  ...
  describe "Rubyagent.tool block shorthand" do
    let(:tool) do
      Rubyagent.tool(:greet, "Greets a person", name: "The person's name") do |name:|
        "Hello, #{name}!"
      end
    end
  ...
  describe Rubyagent::ManagedAgentTool do
    describe ".for" do
      it "creates an anonymous subclass per agent with isolated class state" do
        agent1 = instance_double(Rubyagent::Agent, name: "agent_one", description: "First agent")
        agent2 = instance_double(Rubyagent::Agent, name: "agent_two", description: "Second agent")
  ...
  describe Rubyagent::FinalAnswerTool do
