...
RSpec.describe "prompt customization" do
  let(:model) { instance_double(Rubyagent::Model) }

  def final_answer_response(answer)
    Rubyagent::ChatMessage.new(
      role: "assistant",
      content: "Thought: Done.\nCode:\n```ruby\nfinal_answer(answer: #{answer.inspect})\n```",
      token_usage: Rubyagent::TokenUsage.new(input_tokens: 10, output_tokens: 20)
    )
  end

  describe Rubyagent::PromptTemplates do
    it "defaults all fields to nil" do
      pt = Rubyagent::PromptTemplates.new
    ...
    it "accepts keyword arguments" do
      pt = Rubyagent::PromptTemplates.new(system_prompt: "custom")
  ...
  describe "instructions" do
    it "appends instructions to system prompt" do
      ...
      agent = Rubyagent::CodeAgent.new(model: model, instructions: "Always answer in French.")
      agent.run("Hello")
    ...
    it "appends instructions for ToolCallingAgent" do
      response = Rubyagent::ChatMessage.new(
        role: "assistant",
        content: "Done.",
        token_usage: Rubyagent::TokenUsage.new(input_tokens: 10, output_tokens: 20),
        tool_calls: [
          Rubyagent::ToolCall.new(
            id: "call_1",
            function: Rubyagent::ToolCallFunction.new(name: "final_answer", arguments: { "answer" => "ok" })
          )
        ]
      )
      allow(model).to receive(:generate).and_return(response)

      agent = Rubyagent::ToolCallingAgent.new(model: model, instructions: "Be concise.")
      agent.run("Test")
  ...
  describe "custom system prompt" do
    it "substitutes {{tool_descriptions}} in custom prompt" do
      ...
      templates = Rubyagent::PromptTemplates.new(system_prompt: "Custom agent. Tools: {{tool_descriptions}}")
      agent = Rubyagent::CodeAgent.new(model: model, prompt_templates: templates)
      agent.run("Test")
  ...
  describe "custom planning templates" do
    it "uses custom planning_initial prompt" do
      plan_response = Rubyagent::ChatMessage.new(
        role: "assistant",
      ...
      templates = Rubyagent::PromptTemplates.new(planning_initial: "Custom planner")
      agent = Rubyagent::CodeAgent.new(model: model, prompt_templates: templates, planning_interval: 3)
      agent.run("Test")
    end
    ...
    it "uses custom planning_update prompt" do
      ...
      work_response = Rubyagent::ChatMessage.new(
        role: "assistant",
      ...
      templates = Rubyagent::PromptTemplates.new(planning_update: "Updated planner")
      agent = Rubyagent::CodeAgent.new(model: model, prompt_templates: templates, planning_interval: 2, max_steps: 5)
      agent.run("Test")
    end
  end
end
