...
RSpec.describe "Integration with Ollama", :ollama do
  ...
  before do
    allow(Rubyagent::UI).to receive(:welcome).and_call_original
    allow(Rubyagent::UI).to receive(:step_header).and_call_original
    allow(Rubyagent::UI).to receive(:thought).and_call_original
    allow(Rubyagent::UI).to receive(:code).and_call_original
    allow(Rubyagent::UI).to receive(:observation).and_call_original
    allow(Rubyagent::UI).to receive(:error).and_call_original
    allow(Rubyagent::UI).to receive(:step_metrics).and_call_original
    allow(Rubyagent::UI).to receive(:run_summary).and_call_original
    allow(Rubyagent::UI).to receive(:final_answer).and_call_original
    allow(Rubyagent::UI).to receive(:spinner).and_call_original
  end

  describe "CodeAgent" do
    it "solves a simple math problem" do
      agent = Rubyagent::CodeAgent.new(model: "ollama/qwen2.5:3b", max_steps: 5)
    ...
    it "returns RunResult with timing and token info" do
      agent = Rubyagent::CodeAgent.new(model: "ollama/qwen2.5:3b", max_steps: 3)
  ...
  describe "ToolCallingAgent" do
    it "solves a simple task with tool calling" do
      agent = Rubyagent::ToolCallingAgent.new(model: "ollama/qwen2.5:3b", max_steps: 5)
